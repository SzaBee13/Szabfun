(() => {
  ...
  function loadSettings() {
    if (localStorage.getItem("rps_speed"))
      speedRange.value = localStorage.getItem("rps_speed");
    if (localStorage.getItem("rps_elastic"))
      elasticCheckbox.checked = localStorage.getItem("rps_elastic") === "1";
    if (localStorage.getItem("rps_showVel"))
      showVelCheckbox.checked = localStorage.getItem("rps_showVel") === "1";
    if (localStorage.getItem("rps_inputR"))
      inputR.value = localStorage.getItem("rps_inputR") || 30;
    if (localStorage.getItem("rps_inputP"))
      inputP.value = localStorage.getItem("rps_inputP") || 30;
    if (localStorage.getItem("rps_inputS"))
      inputS.value = localStorage.getItem("rps_inputS") || 30;
    if (localStorage.getItem("google_id")) {
  ...
  async function loadSettingsFromDb(googleId) {
    const res = await fetch(
      `${apiUrl}/load/rock-paper-scissors?google_id=${googleId}`
    );
    const json = await res.json();
    if (json.data) {
      if (json.data.speed) speedRange.value = json.data.speed;
      if (json.data.elastic !== undefined)
  ...
  class Bead {
    ...
    step(dt) {
      ...
      if (this.x + this.r > canvas.clientWidth - 4) {
        this.x = canvas.clientWidth - 4 - this.r;
        this.vx = -Math.abs(this.vx);
      }
      if (this.y - this.r < 4) {
        this.y = this.r + 4;
      ...
      if (this.y + this.r > canvas.clientHeight - 4) {
        this.y = canvas.clientHeight - 4 - this.r;
        this.vy = -Math.abs(this.vy);
      }
    }
  }
    ...
    function spawnMany(type, n) {
      for (let i = 0; i < n; i++) {
        ...
        do {
          ...
          tries < 20 &&
          beads.some((b) => (b.x - x) ** 2 + (b.y - y) ** 2 < (b.r + 14) ** 2)
        );
  ...
  function updateLeader() {
    ...
    // --- Reorder leaderboard rows by count ---
    const leaderboard = [
      {
        type: "rock",
        count: counts.rock,
        row: document.getElementById("rowRock"),
      },
      {
        type: "paper",
        count: counts.paper,
        row: document.getElementById("rowPaper"),
      },
      {
        type: "scissors",
        count: counts.scissors,
        row: document.getElementById("rowScissors"),
      },
    ];
    leaderboard.sort((a, b) => b.count - a.count);

    // Parent node of leaderboard rows
    const parent = leaderboard[0].row.parentNode;
    leaderboard.forEach((item) => parent.appendChild(item.row));

    // Check for win condition
    const nonZeroTypes = Object.entries(counts).filter(
      ([type, count]) => count > 0
    );
  ...
  function draw() {
    ...
    grd.addColorStop(0, "#071428");
    grd.addColorStop(1, "#082033");
    ctx.fillStyle = grd;
  ...
  quickButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const val = parseFloat(btn.id.split("-")[1]);
